...
class DDRNet(BaseModule):
    ...
    def forward(self, x):
        ...
        x_c_res = x_c.clone()  # [4, 256, 64, 64]
        x_s_res = x_s.clone()  # [4,128,128,128]
        ...
        x_c_m = x_c.clone()
        # 添加上下文分支残差
        # x_c_res_comp = self.compression_3(self.down_3(self.relu(x_c_res)))  # [4, 1024, 16, 16]
        # x_c = x_c + x_c_res_comp
        #
        # # 添加高分辨率分支残差
        # x_s_res_comp = self.compression_4(self.relu(x_s_res))
        # # 空间高分辨率分支的相加
        # x_s = x_s + x_s_res_comp
        # stage6
        x_c = self.context_branch_layers[3](self.relu(x_c))
